...
#    Updated: 2016/03/25 23:19:41 by alelievr         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
OBJS_S		=	$(patsubst %.c,%.o, $(filter %.c, $(SRCS_SERVER))) \
				...
				$(patsubst %.s,%.o, $(filter %.s, $(SRCS_SERVER)))
OBJ_S		=	$(addprefix $(OBJDIR)/,$(notdir $(OBJS_S)))
OBJS_C		=	$(patsubst %.c,%.o, $(filter %.c, $(SRCS_CLIENT))) \
				$(patsubst %.cpp,%.o, $(filter %.cpp, $(SRCS_CLIENT))) \
				$(patsubst %.s,%.o, $(filter %.s, $(SRCS_CLIENT)))
OBJ_C		=	$(addprefix $(OBJDIR)/,$(notdir $(OBJS_C)))
NORME		=	**/*.[ch]
VPATH		+=	$(dir $(addprefix $(SRCDIR_S)/,$(SRCS_SERVER)))
VPATH		+=	$(dir $(addprefix $(SRCDIR_C)/,$(SRCS_CLIENT)))
...
ifneq ($(filter %.cpp,$(SRC_SERVER)$(SRC_CLIENT)),)
	LINKER = $(CXX)
endif
...
$(NAME_CLIENT): $(OBJ_C)
	@$(if $(findstring libft,$(LDLIBS)),$(call color_exec_t,$(CCLEAR),$(CCLEAR),\
		make -j 4 -C libft))
	@$(call color_exec,$(CLINK_T),$(CLINK),"Link of $(NAME_CLIENT):",\
		$(LINKER) $(CFLAGS) $(LDFLAGS) $(LDLIBS) $(OPTFLAGS) $(DEBUGFLAGS) $(LINKDEBUG) $(VFRAME) -o $@ $^)

$(NAME_SERVER): $(OBJ_S)
	@$(if $(findstring libft,$(LDLIBS)),$(call color_exec_t,$(CCLEAR),$(CCLEAR),\
		make -j 4 -C libft))
	@$(call color_exec,$(CLINK_T),$(CLINK),"Link of $(NAME_SERVER):",\
		$(LINKER) $(CFLAGS) $(LDFLAGS) $(LDLIBS) $(OPTFLAGS) $(DEBUGFLAGS) $(LINKDEBUG) $(VFRAME) -o $@ $^)
